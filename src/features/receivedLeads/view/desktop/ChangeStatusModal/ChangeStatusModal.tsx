...
import { changeLeadStatus, changeStatusModal } from 'store/leads/receivedLeads/receivedLeads';
...
export const ChangeStatusModal = () => {
  ...
  const { isOpen, id } = useAppSelector(state => state.receivedLeads.modal.statusModal);
        ...
        <div className={b('modal')}>
          ...
          <div className={b('modal-buttons')}>
            <div className={b('modal-button')}>
              <Button color="hollow-blue" onClick={sendStatus} disabled={!statusActive}>
                {locale.done}
              </Button>
            </div>
          </div>
