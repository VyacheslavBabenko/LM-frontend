...
const FilterHeader = ({
	...
	const handleKeyUp = (e: React.KeyboardEvent<HTMLElement>) => {
		...
		if (e.key === "Enter" && onSubmit) {
			onSubmit(
				new Event("submit") as never as React.FormEvent<HTMLFormElement>
			);
		}
	};
	...
	return (
		...
		>
			<div className={b("top")}>
				<div
					className={b("block", { clickable: true })}
					onClick={() => setIsOpen(!isOpen)}
				>
					<SVG
						className={b("icon", { position: "right" })}
						svgProps={{ src: filterSVG }}
					/>
					{locale.filter}
					{!!children && (
						<SVG
							className={b("icon", { close: !isOpen, position: "left" })}
