...
function TableBody<T, I = null>(props: TTableBody<T, I>) {
  const { config, data, withTotal, configInner, dataInner, mapFnInner, callback } = props;
      ...
      {data.map((item, index) => {
        ...
        return (
          <React.Fragment key={index}>
            ...
            {dataInner && mapFnInner && configInner && indexOpened === index && (
              <tr>
                <td colSpan={config.length}>
                  <div className={b('table-inner')}>
                    <TableInner data={dataInner} mapFn={mapFnInner} config={configInner} withTotal />
                  </div>
                </td>
              </tr>
            )}
