...
const Select = function <TValue = string>({
				...
				.map((item) => (
					<li
						...
						onClick={() => {
							...
							const checkHasActive = changedList.filter(
								(item2) => item2.active
							);
					...
					>
						{locale[item.renderElement as keyof typeof locale] ||
							item.renderElement}
					</li>
				)),
	...
	return (
		<div
			className={b({ open: isOpen, color, capitalized })}
			onClick={() => changeOpen(!isOpen)}
		>
			<div className={b("item-selected")}>
				<div className={b("item-value-selected")}>
					{locale[activeItem.selectedRenderElement as keyof typeof locale] ||
